...
//----------------------------------------------------------------------------------
    ...
    Scene_File.prototype.create = function() {
        ...
        this._listWindow.height = 450;
		//ステータスウインドウの配置
        this._statusWindow = new Window_SavefileStatus(406, 127, 330, 450);
...
Window_SavefileList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, 86, 126, 320, 450);
    this.activate();
    this._mode = null;
};

Window_SavefileList.prototype.itemHeight = function() {
    return this.standardFontSize() + 10;
};
...
//現状[ファイル名+ID、プレイ時間、地名、持ち金]、名称変更は''の中の文字で
    Window_SavefileStatus.prototype.drawFileId = function(id, x, y) {
		...
		this.drawText('Location：' + ' ', 2, 270, 80);
		//this.drawText('Gold：' + ' ', 70, 260, 80);
		this.drawText('PlayTime：' + ' ', 2, 300, 80);
    };

    Window_SavefileStatus.prototype.drawContents = function(info, x, valid) {
        this.drawText(info.playtime, 80, 300, 200);
		
		if (info.location) {
		this.drawText(info.location, 80, 270, 200);
		}
		
	//	if (info.level) {
	//		this.drawText(TextManager.levelA + " " + info.level, 70, 320, 60);
	//	}
	//	if (typeof info.gold === 'number') {　//値が数字の時に通る
		//	this.drawText(info.gold + " " + TextManager.currencyUnit, 150, 260, 200);
	//	}


		if (valid) {
            this.drawPartyCharacters(info, 80, 400);
        }
...
Window_SavefileStatus.prototype.drawSnappedImage = function(info, rect, valid){
    ...
    var dh = 0;
    var dw = 0;
    var dx = (rect.width - bitmap.width)/2;　//表示位置X
